<?php
    ...
    include_once "config.php";
        ...
        $query = "SELECT * FROM song WHERE sname LIKE '%{$search_value}%' OR sartist LIKE '%{$search_value}%' ";
        $sql = mysqli_query($conn, $query);
        $output = "";
        if(mysqli_num_rows($sql) > 0){
            while($row = mysqli_fetch_assoc($sql))
            {
                $output .= '<a href="play.php?ssid='.$row['sid'].'"><li>
                                <div class="s-row">
                                <div class="song-info">
                                    ...
                                    <audio id="audio-d" controller controlsList="nodownload" class="song" src="Tracks/'.$row['ssong'].'.mp3"></audio>
                                    <span id="song-d" class="audio-duration">
                                    <script>
            ...
            $output = '<h4 class="text-muted" style="text-align: center;">No match found for - " '.$search_value.' "<h5>';
        }
        echo $output;
