<?php
    ...
    {
        $query = "DELETE FROM songpl WHERE sid = '{$songid}' AND plid = '{$plid}' AND uid = '{$_SESSION['uid']}'";
        $sql2 = mysqli_query($conn, $query);
        if($sql2){
            $query = "SELECT * FROM songpl JOIN song WHERE uid = {$_SESSION['uid']} AND plid = {$plid} AND song.sid = songpl.sid";
