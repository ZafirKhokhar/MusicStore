    ...
    <div class="main">
        ...
        <div class="right-container">
          <div class="heading">
            ...
            <div class="addpl">
              <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">Create New Playlist <i style="font-size: 30px; text-align:center;" class="fas fa-plus"></i></button>
            </div>
          </div>
...
<footer class="text-center text-lg-start bg-light text-muted">
  ...
  <!-- Footer -->
    ...
    <script>
      ...
      function createPL() {
          let searchTerm = cPlaylist.value;
            if(searchTerm != ""){
                // cPlaylist.classList.add("active");
                let xhr2 = new XMLHttpRequest();
                xhr2.open("POST","php/createPL.php",true);
                xhr2.onload = ()=>{
      ...
      function removePL(plist){
        let deleteTerm = plist;
            if(deleteTerm != ""){
                // cPlaylist.classList.add("active");
                let xhr2 = new XMLHttpRequest();
                xhr2.open("POST","php/removePL.php",true);
                xhr2.onload = ()=>{
                    if(xhr2.readyState === XMLHttpRequest.DONE){
                        if(xhr2.status === 200){
                            let data = xhr2.response;
                ...
                xhr2.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                xhr2.send("deleteTerm=" + deleteTerm);
        }
      };
      ...
      window.onload = function(){
        ...
        if(userid != ""){
            ...
            xhr.open("POST","php/getplaylist.php",true);
            xhr.onload = ()=>{
